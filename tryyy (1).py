...
mgn.canvas.manager.window.showFullScreen()  # Activation du plein écran pour Qt5
...
pols, = ax.plot([], linestyle='', marker='o', markerfacecolor='w',
                markeredgecolor='w', markeredgewidth=1.0, markersize=10.0, alpha=1.0)
...
with serial.Serial(serial_port, baudrate=9600, bytesize=8, parity='N', stopbits=1, timeout=2) as ser:
    ...
    while True:
        try:
            ...
            ax.draw_artist(pols)
            line1.set_data([int(angle*45) * (np.pi / 180)] * 2, np.linspace(0.0, r_max, 2))
